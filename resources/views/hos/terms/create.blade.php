...
<div class="w-full ">
    <div class="w-full flex justify-center items-center mt-8">
        <form action="{{ route('hosTerms.store') }}" method="post" class="w-1/2">
            @csrf
            @method('POST')
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-2">
                    <label for="grades" class="text-base font-semibold text-custom-black">Grades:</label>
                    <div
                        class="flex flex-wrap border-2 border-dark-gray rounded-md  h-[46px]  w-full justify-evenly gap-4">
                        @foreach ($grades as $grade)
                        <label class="inline-flex items-center">
                            <input type="checkbox" name="grade_id[]" value="{{ $grade->id }}"
                                class="form-checkbox h-5 w-5 text-dark-orange">
                            <span class="font-semibold mr-2 ml-1 uppercase">{{ $grade->name }}</span>
                        </label>
                        @endforeach
                    </div>
                </div>
