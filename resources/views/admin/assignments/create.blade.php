...
@section('Page Title', 'Create Assignments')
...
<div class="w-full">
    <div class="w-full flex justify-center items-center mt-8">
        <form action="{{ route('assignments.store') }}" method="post" class="w-1/2">
            @csrf
            <div class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="date" name="date_assigned" id="date_assigned"
                        class="border-2 border-dark-gray rounded-md p-2 focus:outline-none focus:border-dark-orange"
                        value="{{ old('date_assigned') }}" required />
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <select name="cas_type_id" id="cas_type_id"
                        ...
                        <option value="{{ $cas_type->id }}">{{ $cas_type->name }}: {{ $cas_type->school->name }}
                        </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <select name="term_id" id="term_id"
                        ...
                        <option value="{{ $term->id }}">{{ $term->name }}: {{ $term->grade->name }}</option>
                        @endforeach
                    </select>
                </div>
