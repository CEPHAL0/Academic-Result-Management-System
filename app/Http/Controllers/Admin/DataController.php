...
class DataController extends Controller
{
    ...
    {
        $assignmentCountBySchool = $this->getAssignmentCountsBySchool();
        return view("admin.dashboard.dataIndex", compact("assignmentCountBySchool"));
    }

    public function getAssignmentCountsBySchool()
    {
        $schools = School::all();

        $assignmentData[] = ["School", "Assignment Count"];

        foreach ($schools as $key => $school) {

            $assignmentCount = Assignment::whereHas("subjectTeacher.subject.grade", function ($query) use ($school) {
                return $query->where("school_id", $school->id);
            })->count();

            $assignmentData[$key + 1] = [$school->name, $assignmentCount];
        }

        return json_encode($assignmentData);
    }
    ...
    {
        try {
            $students = Student::all()->sortBy("roll_number");

            $subjects = Subject::all();


            return view("admin.dashboard.dataProfiler", compact("students", "subjects"));
